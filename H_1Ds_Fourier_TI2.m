function [H,f]=H_1Ds_Fourier_TI2(v,mu,B,DeltaS,nband,lambda)
% basis=(ph)x(spin)x(k); use the oppsite Fourier basis for p & h st. PH-sym is obeyed  
%INPUTS:
...
if nnz(DeltaS)==0              %DeltaS - Always need to compute overlaps because of the wrong choice of basis
    HD=sparse(2*N,2*N);
else
